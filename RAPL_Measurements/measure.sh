...
echo "Function,Language,Program,PowerLimit,Package,Core,GPU,DRAM,Time,Temperature,Memory" > $output_file
            ...
            if [ -d "$program" ]; then
                for input in "${INPUT_VALUES[@]}"; do
                    ...
                    if [ -f "$makefile_path" ]; then
                        cd $program
                        make compile
                        make measure INPUT=$input

                        # Append results to measurements.csv
                        temp_file="measurements.csv"
                        tail -n +2 "$temp_file" | sed "s/^/$(basename $program)_$input,/" >> ../../../$output_file
                        make clean
                        cd ../../..
                    else
                        echo "Makefile not found: $makefile_path"
